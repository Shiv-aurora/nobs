...
terraform -chdir="${TF_DIR}" plan \
  ...
  -var='deploy_action_executor=true' \
  -var='deploy_budget_guard=true' \
  -var="agent_image_uri=${AGENT_IMAGE_URI}" \
  -var="action_executor_image_uri=${ACTION_EXECUTOR_IMAGE_URI}" \
...
echo "Private agent, action executor, and budget guard services deployed. Budget guard dry-run: ${DRY_RUN}."
