...
export function PostIdentityBadge({post}: {post: PostLike}): JSX.Element | null {
    ...
    const routeSummary = `${consulted} delegates consulted · ${interrupted} humans interrupted`;
    return <span className='np-post-agent-meta'><span className='np-post-badge is-agent' title={representedName ? `Representing ${representedName} through NoBS` : 'NoBS organizational delegate'}><span className='np-post-badge__mark'/> {label}</span>{modelName.startsWith('gemini') ? <span className='np-post-model' title={modelName}>Gemini</span> : null}{consulted > 0 && (runID ? <button type='button' className='np-post-route-link' onClick={() => window.dispatchEvent(new CustomEvent('noping:open-panel', {detail: {runID}}))} title='Open NoBS route and evidence'>{routeSummary}</button> : <span className='np-post-route-link is-static' title='Seeded demo route summary'>{routeSummary}</span>)}</span>;
}
