...
export function PostIdentityBadge({post}: {post: PostLike}): JSX.Element | null {
    ...
    return <span className='np-post-agent-meta'><span className='np-post-badge is-agent' title={representedName ? `Representing ${representedName} through NoBS` : 'NoBS organizational delegate'}><span className='np-post-badge__mark'/> {label}</span>{consulted > 0 && <button type='button' className='np-post-route-link' onClick={() => window.dispatchEvent(new CustomEvent('noping:open-panel', {detail: {runID}}))} title='Open NoBS route and evidence'>{consulted} delegates consulted · {interrupted} humans interrupted</button>}</span>;
}
